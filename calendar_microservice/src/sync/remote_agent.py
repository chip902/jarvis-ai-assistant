...
class RemoteCalendarAgent:
    ...
    async def load_config(self):
        ...
        try:
            if os.path.exists(self.config_path):
                ...
                self.agent_name = self.config.get("agent_name", f"Agent-{socket.gethostname()}")
                self.environment = self.config.get("environment", self.environment)
                self.central_api_url = self.config.get("central_api_url", self.central_api_url)
                ...
                logger.info(f"Configuration loaded: Agent ID: {self.agent_id}, Name: {self.agent_name}")
            else:
                ...
                logger.info(f"Created new configuration with Agent ID: {self.agent_id}")
    ...
    async def send_heartbeat(self, include_events: bool = False):
        ...
        if not self.central_api_url or not self.agent_id:
            logger.warning("Cannot send heartbeat: missing central_api_url or agent_id")
            return
        ...
        try:
            ...
            async with self.http_session.post(url, json=heartbeat_data) as response:
                if response.status == 200:
                    ...
                    error_text = await response.text()
                    logger.error(f"Failed to send heartbeat: {response.status} - {error_text}")
    ...
    async def collect_all_events(self) -> List[Dict[str, Any]]:
        ...
        for source in self.config.get("calendar_sources", []):
            try:
                ...
                logger.error(f"Error collecting events from source {source.get('name', 'unknown')}: {e}")
    ...
    async def collect_google_events(self, source: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(f"Would collect events from Google Calendar: {source.get('name')}")
    ...
    async def collect_microsoft_events(self, source: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(f"Would collect events from Microsoft Calendar: {source.get('name')}")
    ...
    async def collect_exchange_events(self, source: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(f"Would collect events from Exchange: {source.get('name')}")
    ...
    async def collect_ical_events(self, source: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(f"Would collect events from iCal: {source.get('name')}")
    ...
    async def collect_outlook_events(self, source: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(f"Collecting events from Outlook calendar: {source.get('name')}")
            ...
            calendar_name = source.get('calendar_name', 'Calendar')  # Default to main Calendar folder
            ...
            logger.info(f"Collected {len(events)} events from Outlook calendar '{calendar_name}'")
            return events
            
        except ImportError:
            logger.error("win32com package not available - required for Outlook integration")
            return []
        except Exception as e:
    ...
    async def collect_custom_events(self, source: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(f"Would collect events from custom source: {source.get('name')}")
    ...
    async def run_sync_cycle(self):
        ...
        try:
            ...
            if result:
                logger.info(f"Sync cycle completed: {result.get('message', 'Success')}")
            else:
                logger.warning("Sync cycle completed but no result from central service")
    ...
    async def run(self):
        ...
        try:
            self.running = True
            logger.info(f"Agent started. Sync interval: {self.sync_interval} minutes")
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="Remote Calendar Synchronization Agent")
    parser.add_argument("--config", default="agent_config.json", help="Path to configuration file")
    parser.add_argument("--central-api", default=None, help="URL of central calendar service API")
    parser.add_argument("--once", action="store_true", help="Run sync once and exit")
    parser.add_argument("--interval", type=int, default=None, help="Sync interval in minutes")
    
    args = parser.parse_args()
    
    try:
        # Create and initialize agent
        agent = RemoteCalendarAgent(config_path=args.config, central_api_url=args.central_api)
